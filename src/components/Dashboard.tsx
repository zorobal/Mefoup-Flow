...
export default function Dashboard({
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-900 text-white p-6 rounded-2xl shadow-md">
        <div className="md:col-span-2 space-y-2">
          <h2 className="text-2xl font-black tracking-tight flex items-center gap-2">
            <Sprout className="text-emerald-400 h-7 w-7" />
            MEFOUP-FLOW Cockpit de Pilotage
          </h2>
          <p className="text-xs text-slate-400">
            Console de pilotage intégrée pour Kissine Agro-Industries. Télémétrie géolocalisée d'Obala, calculatrices de rendements et registre des flux comptables unifiés.
          </p>
        </div>
        ...
        <div className="bg-slate-800 p-4 rounded-xl flex items-center justify-between border border-slate-700">
          <div>
            <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-wider">Station Météo : OBALA, Cameroon</span>
            <h4 className="text-base font-black mt-1 flex items-center gap-1">
              <Thermometer className="text-orange-400 h-4 w-4" />
              {meteo.temperatureCelcius}°C • <span className="text-xs font-normal text-slate-300">{meteo.conditionsCiel}</span>
            </h4>
            <p className="text-[10px] text-slate-400 mt-1 flex items-center gap-1">
              <Droplets className="text-blue-400 h-3 w-3" /> Humidité: {meteo.humiditeRecense}% • Pluie: {meteo.risquesPluiePourcent}%
            </p>
          </div>
          <div className="p-2 bg-slate-700/60 rounded-xl">
            {meteo.risquesPluiePourcent > 50 ? <CloudRain className="h-8 w-8 text-blue-400 animate-bounce" /> : <Sun className="h-8 w-8 text-amber-400 animate-spin-slow" />}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        {/* KPI 1 */}
        <div className="bg-white p-4 rounded-xl border shadow-3xs flex items-center justify-between hover:border-emerald-300 transition">
        ...
        {/* KPI 2 */}
        <div className="bg-white p-4 rounded-xl border shadow-3xs flex items-center justify-between hover:border-blue-300 transition">
          <div className="space-y-1">
        ...
        {/* KPI 3 */}
        <div className="bg-white p-4 rounded-xl border shadow-3xs flex items-center justify-between hover:border-indigo-300 transition">
          <div className="space-y-1">
        ...
        {/* KPI 4 */}
        <div className="bg-white p-4 rounded-xl border shadow-3xs flex items-center justify-between hover:border-orange-300 transition">
          <div className="space-y-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border p-4 space-y-3 shadow-3xs">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={financeProgressionData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="Depenses" stroke="#EF4444" strokeWidth={2.5} fillOpacity={1} fill="url(#colorDepenses)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border p-4 space-y-3 shadow-3xs">
          <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1">
      ...
      <div className="bg-white rounded-xl border p-4 space-y-3 shadow-3xs text-xs">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b pb-2">
          <h3 className="font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
            <Shield className="text-indigo-600 h-4.5 w-4.5" />
            Journal d'Audit & Sécurité (Traceabilité d'Audit RG-94)
          </h3>
